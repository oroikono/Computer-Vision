...
def filter_keypoints(img, keypoints, patch_size = 9):
    ...
    # calcuate the threshold for the padding in all directions
    top_bound = patch_size #top
    bottom_bound = h - patch_size #bottom
    left_bound = patch_size #left
    right_bound = w - patch_size #right
   
    # find the in which indexes corners do not 'violate' the setted threshold for image boundaries
    indexes = np.logical_and(np.logical_and(top_bound < keypoints[:, 1], keypoints[:, 1] < bottom_bound),
                          np.logical_and(left_bound < keypoints[:, 0], keypoints[:, 0] < right_bound))
    print(indexes)
    keypointsFLT = keypoints[indexes]
